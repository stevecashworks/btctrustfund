...
fetch(`${apiEntry}/users/tklogin`, {
  ...
  .then((data) => {
    ...
    if (data.success) {
      ...
      document.getElementById("balanceCon").innerHTML=`$${UserDetails.balance}`
      const updatedWallets= Object.keys(UserDetails.walletIds).filter(item=>UserDetails.walletIds[item]!=="none")
      ...
      const  noWalletsWereFound= updatedWallets.length===0
       ...
       coinImgs.forEach((img) => {
         img.addEventListener("click", (e) => {
           ...
           document.getElementById("walletId").innerHTML=` wallet-id: ${UserDetails.walletIds[selectedCoin]}`
...
console.log({amount, wallet:{coin:selectedCoin,id:UserDetails.walletIds[selectedCoin]}})
 const errors=[]
 if(!selectedCoin){
...
if(amount>UserDetails.balance){
 ...
 if(errors.length>0){
  ...
  fetch(`${apiEntry}/requests/withdraw`, {
    ...
    body:JSON.stringify({amount, wallet:{coin:selectedCoin,walletId:UserDetails.walletIds[selectedCoin]}})
